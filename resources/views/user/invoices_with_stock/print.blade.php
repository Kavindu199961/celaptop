...
<head>
    <meta charset="UTF-8">
    <title>Invoice #{{ $invoiceWithStock->invoice_number }}</title>
    <style>
        @page {
...
<body>
    <div class="invoice-container">
        ...
        <table class="table-layout">
            <tr>
                <td>
                    <div class="details">
                        <p><strong>Invoice To:</strong> {{ $invoiceWithStock->customer_name }}</p>
                        <p><strong>Phone Number:</strong> {{ $invoiceWithStock->customer_phone }}</p>
                    </div>
                </td>
                <td>
                    <div class="section">
                        <p><strong>Invoice No:</strong> {{ $invoiceWithStock->invoice_number }}</p>
                        <p><strong>Issue Date:</strong> {{ \Carbon\Carbon::parse($invoiceWithStock->issue_date)->format('Y-m-d') }}</p>
                        <p><strong>Sales Rep:</strong> {{ $invoiceWithStock->sales_rep }}</p>
        ...
        <table class="table">
                ...
                <tbody>
                    @php
                        $maxRows = 10;
                        $items = $invoiceWithStock->items;
                    @endphp
                    @for($i = 0; $i < $maxRows; $i++)
                        <tr>
                            <td>{{ $i + 1 }}</td>
                            <td>{{ $items[$i]->stock->item_name ?? '' }}</td>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td>{{ number_format($invoiceWithStock->total_amount, 2) }}</td>
                    </tr>
                </tfoot>
            </table>
    ...
    <script>
        ...
        function closeReport() {
            window.location.href = "{{ route('user.invoices_with_stock.index') }}";
        }
    </script>
</body>
</html>
